...
import { serverURL, getData, postData } from "../../services/api/FetchNodeServices";
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <Paper elevation={0} style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: matches ? '1fr' : '1fr 1fr', gap: 16 }}>
              <DetailItem icon="ðŸ¢" label="Department" value={job.categoryname} />
              <DetailItem icon="ðŸ“‚" label="Role / Category" value={job.subcategoryname} />
              <DetailItem icon="ðŸ’»" label="Employment Type" value={job.jobtype} />
              <DetailItem icon="ðŸ•" label="Shift / Schedule" value={job.schedule} />
              <DetailItem icon="ðŸ“Š" label="Experience Required" value={job.experience ? `${job.experience} Years` : 'Fresher'} />
              {job.contactperson && <DetailItem icon="ðŸ‘¤" label="Contact Person" value={job.contactperson} />}
            </div>
          </Paper>
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', gap: 8, marginTop: 6, flexWrap: 'wrap' }}>
                      <span style={{ fontSize: 12, color: '#059669', fontWeight: 600 }}>
                        â‚¹{formatSalary(rj.minsalary)}-{formatSalary(rj.maxsalary)}
                      </span>
                      {rj.experience && <span style={{ fontSize: 12, color: '#6b7280' }}>â€¢ {rj.experience} yrs</span>}
                    </div>
                  </div>
                ))}
              </Paper>
