...
### 3. Environment Setup
Create `frontend/.env` and `backend/.env` from their respective `.env.example` files:
...
The DDL script is located at `backend/schema.sql`. It defines:
...
3. In your Render / Production Environment variables, set:
   ...
   - `DB_NAME=defaultdb`
   - `DB_SSL=true`
4. Aiven's public endpoint requires TLS. Set `DB_SSL=true`; the backend enables TLS and, unless `DB_SSL_REJECT_UNAUTHORIZED=true` is set, accepts Aiven's managed certificate chain without requiring a CA file.
...
The backend and frontend are separate projects in this repository. Deploy the backend directory as the Render Web Service; deploy the frontend separately (for example, with Vercel).
...
- **Environment Variables to add in Render Dashboard**:
  ...
  - `DB_NAME`: `defaultdb`
  - `DB_SSL`: `true`
  - `DB_SSL_REJECT_UNAUTHORIZED`: `false` (use `true` only with a CA-trusted certificate chain)
...
3. **Automated Non-Destructive Migrations in `pool.js`**:
   - Added `runAutoMigrations()` inside `backend/routes/pool.js` that checks for legacy schemas before applying DDL statements, guaranteeing compatibility on both fresh and existing local/cloud MySQL instances.
...
1. Ensure MySQL is running and configured in `backend/.env`.
2. Start the backend:
   ```bash
   cd backend
   npm start
